...
<x-slot:fav>Roles-trashed</x-slot:fav>
    <x-slot:title>Roles-trashed</x-slot:title>
    @push('css')
              ...
              <div class="card mb-4">
                ...
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush" id="dataTable">
                    ...
                    @foreach ($softDeletedRoles as $category)
        <tr>
            <td>{{ $loop->iteration }}</td>
            <td>{{ $category->name }}</td>
            <td class="text-nowrap">
                <form action="{{ route('roles.trashed.delete',$category->id) }}" method="POST">
                @can('role-trashed-restore')
                    <a class="btn btn-primary" href="{{ route('roles.trashed.restore',$category->id) }}">restore</a>
                @endcan
                    @csrf
                    @method('DELETE')
                    @can('role-trashed-delete')
