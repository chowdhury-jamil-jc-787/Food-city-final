...
<x-slot:fav>users-trashed</x-slot:fav>
    <x-slot:title>Users-trashed</x-slot:title>
    @push('css')
    ...
    @endif
        ...
        <tr>
            ...
            <td class="text-nowrap">
                <form action="{{ route('users.trashed.delete',$category->id) }}" method="POST">
                @can('user-trased-restore')
                    <a class="btn btn-primary" href="{{ route('users.trashed.restore',$category->id) }}">restore</a>
                @endcan
                    @csrf
                    @method('DELETE')
                    @can('user-trashed-delete')
